...
class Qrcode extends Base
{
    ...
    public function saveInfo()
    {
        ...
        // 更新或添加
        if ($id) {
            $res = Db::name('qrcode')->where('id', $id)->update($data);
        } else {
            $data['add_time'] = time();
    ...
    public function speakerTest()
    {
        ...
        if($brand == 'pinsheng') {
            ...
            if ($result === true) {
                ...
                return errorJson('命令发送失败' . (is_array($result) ? '：' . $result['message'] : ''));
            }
        }
    ...
    public function qrcodeDownload()
    {
        ...
        //设置头信息
        header('Content-Disposition:attachment;filename=' . $qrcodeTitle . '.png');
